...
+ "XY";
        ...
        public static string GetSpells(string hzString, char spliter)
        {
            ...
            for (int j = 0; j < noWChar.Length; j++)
            {
                pyString += spliter + GetSpells(noWChar[j]);
            }
            return pyString.TrimStart(spliter);
        }
        ...
        public static string GetSpell(string str, char spliter)
        {
            ...
            foreach (char c in str)
            {
                ...
                {
                    //字母和符号原样保留
                    tempStr = tempStr + spliter + c.ToString();
                }
                else
                {
                    //累加拼音声母
                    tempStr = tempStr + spliter + GetSpell(c);
                }
            }
            return tempStr.Trim();
        }
        ...
        public static string GetSpell(char c)
        {
            if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
                ...
                return pyFirst[(int)c - 19968].ToString();
            else
                return c.ToString();
        }
