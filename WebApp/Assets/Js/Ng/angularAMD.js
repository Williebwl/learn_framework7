...
define(function () {
    ...
    AngularAMD.prototype.route = function (config) {
        ...
        var load_controller, load_navController, load_toolBarController, load_containerController;
        ...
        */
        ...
        /*
       If `navControllerUrl` is provided, load the provided Url using requirejs.  If `controller` is not provided
       but `navControllerUrl` is, assume that module to be loaded will return a function to act as controller.

       Otherwise, attempt to load the controller using the controller name.  In the later case, controller name
       is expected to be defined as one of 'paths' in main.js.
       */
        if (config.hasOwnProperty('navControllerUrl')) {
            load_navController = config.navControllerUrl;
            delete config.navControllerUrl;
        } else if (typeof config.navController === 'string') {
            load_navController = config.navController;
        }

        // If controller needs to be loaded, append to the resolve property
        if (load_navController) {
            var resolve = config.resolve || {};
            resolve['__AAMDNavCtrl'] = ['$q', '$rootScope', function ($q, $rootScope) { // jshint ignore:line
                var defer = $q.defer();
                require([load_navController], function (ctrl) {
                    defer.resolve(ctrl);
                    $rootScope.$apply();
                });
                return defer.promise;
        ...
        /*
       If `toolBarControllerUrl` is provided, load the provided Url using requirejs.  If `controller` is not provided
       but `toolBarControllerUrl` is, assume that module to be loaded will return a function to act as controller.

       Otherwise, attempt to load the controller using the controller name.  In the later case, controller name
       is expected to be defined as one of 'paths' in main.js.
       */
        if (config.hasOwnProperty('toolBarControllerUrl')) {
            load_toolBarController = config.toolBarControllerUrl;
            delete config.toolBarControllerUrl;
        } else if (typeof config.toolBarController === 'string') {
            load_toolBarController = config.toolBarController;
        }

        // If controller needs to be loaded, append to the resolve property
        if (load_toolBarController) {
            var resolve = config.resolve || {};
            resolve['__AAMDToolBarCtrl'] = ['$q', '$rootScope', function ($q, $rootScope) { // jshint ignore:line
                var defer = $q.defer();
                require([load_toolBarController], function (ctrl) {
                    defer.resolve(ctrl);
                    $rootScope.$apply();
                });
                return defer.promise;
        ...
        /*
       If `containerControllerUrl` is provided, load the provided Url using requirejs.  If `controller` is not provided
       but `containerControllerUrl` is, assume that module to be loaded will return a function to act as controller.

       Otherwise, attempt to load the controller using the controller name.  In the later case, controller name
       is expected to be defined as one of 'paths' in main.js.
       */
        if (config.hasOwnProperty('containerControllerUrl')) {
            load_containerController = config.containerControllerUrl;
            delete config.containerControllerUrl;
        } else if (typeof config.containerController === 'string') {
            load_containerController = config.containerController;
        }

        // If controller needs to be loaded, append to the resolve property
        if (load_containerController) {
            var resolve = config.resolve || {};
            resolve['__AAMDContainerCtrl'] = ['$q', '$rootScope', function ($q, $rootScope) { // jshint ignore:line
                var defer = $q.defer();
                require([load_containerController], function (ctrl) {
                    defer.resolve(ctrl);
                    $rootScope.$apply();
                });
                return defer.promise;
        ...
        app.config(
            ['$controllerProvider', '$compileProvider', '$filterProvider', '$animateProvider', '$provide', '$injector', function (controllerProvider, compileProvider, filterProvider, animateProvider, provide, injector) {
                ...
                angular.extend(onDemandLoader, {
                    ...
                    decorator: function (name, constructor) {
                        provide.decorator(name, constructor);
                        return this;
                    },
    ...
    // .decorator
    AngularAMD.prototype.decorator = executeProvider('decorator');
    // .animation
