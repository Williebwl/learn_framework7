    ...
    public sealed class DBAnalyzer : IDBAnalyzer
    {
        ...
        public string Select()
        {
            if (this.select == null)
            {
                ...
                {
                    //第一页使用TOP
                    this.select = string.Format(@"SELECT TOP {3} * FROM ({1}) DBPager {0}",
                        orderBy, this.SelectWithoutOrderBy, skip, skip + take);
                }
                else
                {
                    //第二页之后使用ROW_NUMBER
                    this.select = string.Format(@"SELECT * FROM (SELECT ROW_NUMBER() OVER ({0}) DBPager_Index, * FROM
({1}) DBPager_TMP
) DBPager WHERE DBPager_Index>{2} AND DBPager_Index<={3}",
                        orderBy, this.SelectWithoutOrderBy, skip, skip + take);
                }
            }
            return this.select;
